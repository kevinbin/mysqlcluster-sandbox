...
hostname="localhost"
if [ "${initial}" = "1" ]; then
       printf "%s: %-80s\n" "sandbox" "Starting data node (nodeid=2) - initial node start"
      $libexec/ndbd --ndb-nodeid=2 -c "localhost:1186" --initial
else
       printf "%s: %-80s\n" "sandbox" "Starting data node (nodeid=$id)"
      $libexec/ndbd --ndb-nodeid=2 -c "localhost:1186"
fi
printf "%s: %-80s\n" "sandbox" "Data node 2 is STARTING"
if [ "${nowait}" = "1" ]; 
then 
   exit; 
fi
...
do
   sleep 1
   st=`$bindir/ndb_mgm -c "$connectstring" -e "2 STATUS" |grep "Node 2: started"`
   if [ "${st}" != "" ]; 
   then
             break;
   else
...
printf "\n"
printf "%s: %-80s\n" "sandbox" "Data node 2 is STARTED"
